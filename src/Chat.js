...
const Chat = props => {
  ...
  const handleMessageSubmit = message => {
    ...
    axios
      .get()
    //https://magurodev.herokuapp.com/chatbot
    axios
      .post("http://localhost:3111/chatbot", data)
      .then(response => {
        const responseData = {
          text: response.data["message"]["fulfillmentText"],
          //   text: response.data["message"]["fulfillmentText"] != "" ? response.data["message"]["fulfillmentText"] : "",
          isBot: true
        };
